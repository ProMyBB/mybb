...
class MyBB
{
	 ...
	 * Turkish Translate: https://mybb.pro - MyBB.Pro
	 */
	function trigger_generic_error($code)
	{
		...
		switch ($code) {
			case "cache_no_write":
				$message = "Önbellek dizini (cache/) klasörü var olmalı ve sunucunuz tarafından yazılabilir olmalıdır.İlgili dosyanın izinlerini yazılabilir olacak şekilde değiştirin. (Unix tabanlı sunucularda CHMOD değeri 777 olmalı.)";
				$error_code = MYBB_CACHE_NO_WRITE;
				break;
			case "install_directory":
				$message = "Kurulum dizini (install/) hala sunucunuzda mevcut ve kilitlenmiş değil. MyBB'ye erişmek için lütfen bu dizini kaldırın veya içinde 'lock' adında boş bir dosya oluşturun.";
				$error_code = MYBB_INSTALL_DIR_EXISTS;
				break;
			case "board_not_installed":
				$message = "Forumunuz henüz kurulmamış ve yapılandırılmamış.Lütfen, göz atmaya çalışmadan önce kurulumu yapın.";
				$error_code = MYBB_NOT_INSTALLED;
				break;
			case "board_not_upgraded":
				$message = "Forumunuz henüz güncellenmedi. Lütfen, göz atmaya çalışmadan önce forumunuzu güncelleyin.";
				$error_code = MYBB_NOT_UPGRADED;
				break;
			case "sql_load_error":
				$message = "MyBB SQL uzantısını yükleyemedi. Lütfen, destek için MyBB.Pro ile iletişime geçin.<a href=\"https://mybb.pro\">MyBB.Pro Destek</a>";
				$error_code = MYBB_SQL_LOAD_ERROR;
				break;
			case "apc_load_error":
				$message = "APC önbellek desteğini kullanabilmek için APC'nin PHP ile yapılandırılması gerekir.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "apcu_load_error":
				$message = "APCu önbellek desteğini kullanabilmek için APCu'nun PHP ile yapılandırılması gerekir.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "eaccelerator_load_error":
				$message = "eAccelerator önbellek desteğini kullanabilmek için eAccelerator'ün PHP ile yapılandırılması gerekir.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "memcache_load_error":
				$message = "Sunucunuzda memcache desteği etkin değil.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "memcached_load_error":
				$message = "Sunucunuzda memcached desteği etkin değil.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "xcache_load_error":
				$message = "Xcache önbellek desteğini kullanabilmek için Xcache'in PHP ile yapılandırılması gerekir.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			case "redis_load_error":
				$message = "Sunucunuzda redis desteği etkin değil.";
				$error_code = MYBB_CACHEHANDLER_LOAD_ERROR;
				break;
			default:
				$message = "MyBB dahili bir hatayla karşılaştı. Lütfen, destek için MyBB.Pro ile iletişime geçin.<a href=\"https://mybb.pro\">MyBB.Pro Destek</a>";
