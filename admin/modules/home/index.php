...
if ($mybb->input['action'] == "version_check") {
	...
	$contents = fetch_remote_file("https://mybb.pro/surum-kontrol/version_check.php");
	...
	$feed_parser->parse_feed("https://mybb.pro/syndication.php?fid=35&limit=15");
	...
	if (isset($update_check['latest_version_code']) && $update_check['latest_version_code'] > $mybb->version_code) {
		$lang->new_version_available = $lang->sprintf($lang->new_version_available, "MyBB {$mybb->version}", "<a href=\"https://mybb.pro/forum-mybb-turkce-kurulum-paketleri\" target=\"_blank\" rel=\"noopener\">MyBB {$update_check['latest_version']}</a>");
